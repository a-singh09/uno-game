...
const convertToGameComponentState = (
  offChainState: OffChainGameState,
  ...
  return {
    ...
    player5Deck: [],
    player6Deck: [],
    playedCardsPile: offChainState.lastPlayedCardHash
      ? [offChainState.lastPlayedCardHash]
      : [],
    drawCardPile: [],
...
const Room = () => {
  ...
  const initializeComputerGame = async () => {
    ...
    try {
      console.log(
        "Initializing computer game directly without blockchain transaction..."
      );

      // Skip the blockchain startGame transaction for computer mode
      // Just initialize the game state directly
      console.log(
        "Computer game initialized - skipping blockchain transaction"
      );
      ...
      console.log(
        "Computer game action hash (not committed to blockchain):",
        actionHash
      );

      // Skip the blockchain commitMove transaction for computer mode
      console.log(
        "Skipping blockchain commitMove transaction for computer mode"
      );
      ...
      if (account) {
        try {
          ...
          if (contractResult.contract && id) {
            ...
            if (gameState) {
              console.log(
                "Game state fetched successfully, setting state:",
                gameState
              );
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log(
        "All required data available, initializing computer game (without blockchain transactions)..."
      );
      console.log(
        "Contract details available (but not used for transactions):",
        contract
      );
  ...
  return !roomFull ? (
    ...
    >
      ...
      {isComputerMode ? (
        ...
        (() => {
          console.log(
            "Rendering computer mode, gameStarted:",
            gameStarted,
            "currentUser:",
            currentUser
          );
