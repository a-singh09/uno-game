...
import {
  updateGlobalCardHashMap,
  getGlobalCardHashMap,
  getCardFromGlobalHashMap,
} from "./globalState";
...
export function isValidPlay(
  cardHash: string,
  {
    currentColor,
    currentValue,
  }: { currentColor: CardColor; currentValue: CardValue }
): boolean {
...
export function canPlay(
  ...
  return handHashes.some((cardHash) => {
    const card = getCardFromHash(cardHash);
    const ans = card
      ? isValidPlay(cardHash, { currentColor, currentValue })
      : false;
    return ans;
  });
}
...
export function initializeOffChainState(
  gameId: bigint,
  players: string[]
): OffChainGameState {
...
export function applyActionToOffChainState(
  state: OffChainGameState,
  action: Action
): OffChainGameState {
  const newState = { ...state };
      ...
      if (action.cardHash) {
        ...
        if (playedCard) {
          ...
          if (playedCard.value === "skip") {
            ...
            skipNextPlayer = true; // Player who draws also skips turn
            console.log(
              "Wild Draw 4 card played - next player draws 4 and skips"
            );
          }
          ...
          console.error(
            `Played card with hash ${action.cardHash} not found in global card hash map`
          );
        }
      }
      break;
    case "drawCard":
      if (action.player === state.players[state.currentPlayerIndex]) {
        try {
          ...
          const wasPlayed = isValidPlay(drawnCardHash, {
  ...
  if (reverseDirection) {
    newState.direction =
      newState.direction === "clockwise" ? "counterclockwise" : "clockwise";
  }

  // Calculate next player index based on direction and skip
  let nextPlayerIncrement = newState.direction === "clockwise" ? 1 : -1;

  // If skip card was played, skip one more player
  if (skipNextPlayer) {
    nextPlayerIncrement = newState.direction === "clockwise" ? 2 : -2;
  }
...
export function hashAction(action: Action): string {
  ...
  const encodedAction = abiCoder.encode(
    ...
    [action.type, action.player, action.cardHash || ""]
  );
...
export function storePlayerHand(
  gameId: bigint,
  playerAddress: string,
  handHashes: string[]
): void {
