...
app.get("/api/game-state/:gameId", (req: Request, res: Response) => {
  try {
    ...
    logger.error(
      `Error retrieving game state for game ID ${req.params.gameId}:`,
      error
    );
...
app.get("/api/recent-games", (req: Request, res: Response) => {
  try {
...
io.on("connection", (socket: Socket) => {
  activeConnections++;
  logger.info(
    `User ${socket.id} connected. Active connections: ${activeConnections}`
  );
    ...
    (
      {
        ...
        gameId,
        playerAddress,
      }: { room: string; gameId?: string; playerAddress?: string },
      callback: (response: any) => void
    ) => {
      try {
        logger.info(
          `User ${socket.id} attempting to rejoin room ${room} with walletAddress ${playerAddress}`
        );
        ...
        const roomExists =
          roomUsers.length > 0 || gameStateManager.hasGameState(gameRoomId);

        if (roomExists) {
          // Try to find the user's previous session by matching disconnected users
          let reconnectedUser: any = null;
          const disconnectedUsers = roomUsers.filter(
            (u: User) => u.connected === false
          );
          ...
          if (disconnectedUsers.length > 0) {
            ...
            // If multiple, try to match by playerAddress or take the most recent
              ...
              reconnectedUser = disconnectedUsers.find(
                (u) => u.walletAddress === playerAddress
              );
            }

            // Fallback: take the most recently disconnected user
            if (!reconnectedUser && disconnectedUsers.length > 0) {
              reconnectedUser = disconnectedUsers.sort(
                (a: User, b: User) => (b.disconnectedAt || 0) - (a.disconnectedAt || 0)
              )[0];
            }
          }
          ...
          if (reconnectedUser) {
            ...
            if (playerAddress && !reconnectedUser.walletAddress) {
              reconnectedUser.walletAddress = playerAddress;
            }
            logger.info(
              `Reconnected user ${socket.id} as ${reconnectedUser.name} in room ${room}`
            );
          }
          ...
          const updatedRoomUsers = getUsersInRoom(room).filter(
            (u: User) => u.connected !== false
          );
          io.to(room).emit("roomData", { room, users: updatedRoomUsers });
          logger.info(
            `Sent updated room data to room ${room} with ${updatedRoomUsers.length} users`
          );
        } else {
          logger.warn(
            `Room ${room} not found for rejoin (no users and no game state)`
          );
          if (callback && typeof callback === "function") {
  ...
  socket.on(
    ...
    ({ roomId, gameId }: { roomId: string; gameId?: string }) => {
      try {
        logger.info(
        ...
        if (!gameState && gameId) {
          ...
          if (gameData) {
            ...
            cardHashMap = gameData.cardHashMap;
            logger.info(
              `Game state restored from persistent storage for game ${gameId}`
            );
          }
        }
  ...
  socket.on(
    "join",
    (payload: { room: string; walletAddress?: string }, callback: (error?: string) => void) => {
      // Filter to only count connected users for room capacity
      const connectedUsersInRoom = getUsersInRoom(payload.room).filter(
        (u: User) => u.connected !== false
      );
      ...
      const activeUsers = getUsersInRoom(newUser!.room).filter(
        (u: User) => u.connected !== false
      );
      ...
      if (reconnected) {
        logger.info(
          `User ${socket.id} reconnected as ${newUser!.name} in room ${
            newUser!.room
          }`
        );
      } else {
        logger.info(
          `New user ${socket.id} joined as ${newUser!.name} in room ${
            newUser!.room
          }`
        );
      }
  ...
  socket.on("requestGameInit", (payload: any) => {
    ...
    if (user) {
      ...
      logger.info(
        `Initializing game in room ${user.room} with ${numPlayers} players`
      );
  ...
  socket.on("initGameState", (gameState: any) => {
    ...
    if (user) {
      ...
      io.to(user.room).emit("initGameState", gameState);
      logger.info(
        `Game initialized in room ${user.room} with ${
          Object.keys(gameState).filter((k) => k.includes("Deck")).length
        } players`
      );
    }
  });
  ...
  socket.on("disconnect", (reason: string) => {
    activeConnections--;
    logger.info(
      `User ${socket.id} disconnected: ${reason}. Active connections: ${activeConnections}`
    );
    ...
    if (user) {
      ...
      setTimeout(() => {
        ...
        if (currentUser && currentUser.connected === false) {
          ...
          if (removedUser) {
            logger.info(
              `User ${socket.id} did not reconnect, removing from room ${removedUser.room}`
            );
